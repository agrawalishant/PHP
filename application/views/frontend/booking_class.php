    ...
    <section id="features-app" class="padd-80 head-section">
          ...
          <div class="container">
              <?php if($this->session->flashdata('bookedmsg')!='') { ?>
                ...
                <div id="showmessg">
                    <h4>Name:- <?php echo ucfirst($result[0]['name']); ?> </h4>
                </div>
                <?php } ?>
                            ...
                            <table class="table table-bordered table-striped col-md-12 amtcor">
                                <tbody class="dv-tm checks" >
                                    ...
                                    <tr>
                                        <td>
                                            <?php $chg=$price; 
                                                $totalHour = $value['end_time'] - $value['start_time'];
                                            ?>
                                        ...
                                        <?php echo $chg*$totalHour; ?>&nbsp&nbsp&nbsp&nbsp<?php echo "(<i class='fa fa-gbp'></i>".$chg."/hour)" ; } ?>    
                                        </td>
                                ...
                                <tbody class="dv-tm checks" >
                                    <tr>
                                        ...
                                        <td class="td-01">
                                            ...
                                            <i class='fa fa-gbp'></i><?php echo $chg*$totalHour;  ?>&nbsp&nbsp&nbsp&nbsp <?php echo "(<i class='fa fa-gbp'></i>".$chg."/hour)" ; ?>
                                            <input type="hidden"  class="addchr" name="addchr[]" value="<?php echo $chg*$totalHour; ?>" />
                    ...
                    <div class="row">
                        <div class="col-md-12" style='display:flex;'>
                            ...
                            <form action="<?php echo site_url('Student/mmsgateway');?>" id="slotbookingform" method="post">
                                <input type="hidden" name="postcode" value='<?php echo $srcpostcode;?>' id="postcode_data" />
                                ...
                                <!--<input type="hidden" name="mms_payment" value='<?php //echo 'mms_payment';?>' id="form_btn_id"  />-->
                                ...
                                <input type="button" class="btn btn-primary" name="submit_timeslot" value="Book Now" id="book_now_btn" onclick="goMMSpayment();" />
                            </form>
                            <form action="<?php echo site_url('Products/buySlots');?>" id="slotbookingformpaypal" method="post">
                                <input type="hidden" name="postcode" value='<?php echo $srcpostcode;?>' id="postcode_data" />
                                <input type="hidden" name="timeslots_data[]" value='' id="timeslots_data_p" />
                                <input type="hidden" name="bkdates_data[]" value='' id="bookingdate_data_p" />
                                <input type="hidden" name="totalcharges_data" value='' id="totalcharges_data_p" />
                                <input type="hidden" name="inst_id" value='<?php echo $inst_id;?>' id="instructor_id_p"  />
                                <!--<input type="hidden" name="paypal_payment" value='<?php //echo 'paypal_payment';?>' id="form_btn_id"  />-->
                                <input type="hidden" name="randam_id" value='' id="randam_id_p"  />
                                <?php //$stu_id = $this->session->userdata('student_session'); //$st_id = $stu_id['res'][0]['id']; //echo $st_id; ?> 
                                <input type="hidden" name="stu_id" value='<?php echo $st_id;?>' id="student_id_p"  />
                                <!--<input type="submit" class="btn btn-primary" name="submit_timeslot" value="Book Now" id="book_now_btn" />-->
                                <input type="button" class="btn btn-primary" name="submit_timeslot" value="PayPal" id="book_paypal_btn" onclick="gopaypal();" style="margin-top: 20px;margin-left: 15px;"/>
                            </form>
                        </div>
                    </div>  
                    <!-- content tab-->
...
<script type="text/javascript">
  ...
  function gopaypal()
    {
        var bkdates_data = document.getElementById('bookingdate_data_p').value;
        var timeslots_data = document.getElementById('timeslots_data_p').value;
        var totalcharges_data = document.getElementById('totalcharges_data_p').value;
        var inst_id = document.getElementById('instructor_id_p').value;
        var student_id = document.getElementById('student_id_p').value;
        //var randam_id = document.getElementById('randam_id_p').value;rad_id
        ...
        if(student_id >0)
        {
            ...
            {
                $('#randam_id_p').val(randam_id);
                $('#slotbookingformpaypal').submit();   
            }
            else
            {
                return false;
            ...
            $.ajax({
                ...
                success: function(response)
                {
                    ...
                    else
                    {
                        ...
                        document.getElementById('student_id_p').value=response.studentid;
    ...
    function myfunc()
    {
        ...
        $.ajax({
                ...
                method: 'post',
                data: {dates: date, instid : id, price: price, vtype: Vechical, AtoS:arrToString },
                dataType: 'json',
                success: function(response)
                {
     ...
     $(document).ready(function(){
        ...
        $('#book_paypal_btn').attr("disabled",true);
    });     
    ...
    $(document).on('click','.checks',function(){
        ...
        else
        {
            ...
            else
            {
             ...
             $('#book_paypal_btn').attr("disabled",false);
                var timeslots='';
                var bkdate=''
                var addcharges=0;
                var come;
                        ...
                        $.ajax({
                                ...
                                //console.log(response);
                                    if(response.success == 'true')
                                    {
                                        ...
                                        $('#timeslots_data_p').val(response.time);
                                        $('#bookingdate_data_p').val(response.date);
                                        $('#totalcharges_data_p').val(response.amt);
                                        $('#randam_id_p').val(response.randam);
                                    }
                                    else
                                    {
                                        $('#book_now_btn').attr("disabled",true);
