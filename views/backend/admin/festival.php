        ...
        <div class="row mb-2">
          <div class="col-sm-6">
            ...
            <?php 
              ...
              if(!in_array('festivaladd',$chpermission)){  
            ?>
                ...
                $sno++;
                 ?>
                  <tr>
                    ...
                    <td><?php echo $a = word_limiter(strip_tags($datadisp['description']), 5);?></td>
                  
                    <td><?php echo $datadisp['date'];?></td>
                      <td>
                        <?php
                          $chpermission = fetchpermission($id);
                          if(!empty($chpermission)){
                          if(!in_array('festivalview',$chpermission)){ 
                        ?>
                        <a  href="" data-toggle="modal" data-target="#modelview<?php echo $datadisp['festival_id'];?>" data-id="<?php echo $datadisp['festival_id'];?>" >  <i class="fas fa-eye"></i></a>

                        <?php } if(!in_array('festivaledit',$chpermission)){ ?>

                        <a  href="" data-toggle="modal" data-target="#model<?php echo $datadisp['festival_id'];?>" data-id="<?php echo $datadisp['festival_id'];?>" >  <i class="fas fa-edit"></i></a>
                        
                        <?php } if(!in_array('festivaldelete',$chpermission)){ ?>
                      
                       <a onclick="return confirm('Are you sure you want to Delete this item?');" href="<?php echo base_url();?>deletefestival/delete/<?php echo $datadisp['festival_id'];?>" > <i class="fas fa-trash"></i></a>
                     <?php } } ?>
                     </td>
                    </tr>
                    <?php endforeach;?>
...
<!-- Modal -->
<div class="modal fade" id="model<?php echo $iddata=$viewData['festival_id'];?>" role="dialog">
    <div class="modal-dialog">
        <div class="modal-content">
            <!-- Modal Header -->
            <div class="modal-header">
...
<!-- Modal -->
<div class="modal fade" id="modelview<?php echo $iddata=$viewData['festival_id'];?>" role="dialog">
    <div class="modal-dialog">
        <div class="modal-content">
            <!-- Modal Header -->
            <div class="modal-header">
            <h4 class="modal-title" id="myModalLabel">View Festival</h4>
                <button type="button" class="close" data-dismiss="modal">
                    <span aria-hidden="true">&times;</span>
                    <span class="sr-only">Close</span>
                </button>
