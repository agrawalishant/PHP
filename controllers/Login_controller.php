...
//$this->load->library('facebook', array('appId' => '1286483181750813', 'secret' => 'e4595a99f7aa866797d130f4e6ef848f'));
    }
...
public function astrologer_logout()
{
    ...
    $idastro = $this->session->userdata('astro_id');
    $data2 = [
            'astro_online_status' => 0,
            'astro_online_chat_status' => 0, 
            'astro_online_call_status' => 0 
          ];
    $online = updateData('astrologers',$data2,array('astro_id'=>$idastro));
...
function detailsfill()
{
  ...
  $this->load->view('frontend/include/header_detail.php');
  $this->load->view('frontend/pagesuser/updatedetails');
}
//-------------------------------- this is for user logout use only-------------------------------------------------------------
// --------------------------------this is for user logout use only-------------------------------------------------------------
...
public function user_logoutforreject()
{
    //   online status start
    $dataonline['user_online_status']='0';
    $online=$this->db->where('user_id',$this->session->userdata('user_id'))
...
// {
// $Times = date('Y-m-d H:i:s', strtotime('-5 min'));

// //$loggeduserid = $this->session->userdata['indirasess']['loggeduserid'];
// $loggedastroid = $this->session->userdata('astro_id');
// $dataes = 'sc_id,sc_lastping';
// // $where = "sc_lastping < $Times AND sc_login_status = 1 AND sc_astro_id = $loggedastroid";
// $where = array('sc_lastping <'=> $Times, 'sc_login_status'=>1, 'sc_astro_id'=>$loggedastroid);
// $exist=$this->Generalmodel->get_data_by_condition($dataes,'statuscheck',$where);
...
//             'astro_online_call_status' => 0 
//           ];
//     $last = updateData('astrologers',$data2,array('astro_id'=>$result[0]['sc_astro_id']));
//     if(!empty($last))
//     {
//       $this->session->unset_userdata('astro_id');
//     }
